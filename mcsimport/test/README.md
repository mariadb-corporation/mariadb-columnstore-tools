...
This is fine for local ColumnStore installations on Linux. On Windows and for remote tests these parameters need to be overwritten through the environment variables `COLUMNSTORE_INSTALL_DIR`, `MCSAPI_CS_TEST_IP`, `MCSAPI_CS_TEST_PORT`, `MCSAPI_CS_TEST_USER`, and `MCSAPI_CS_TEST_PASSWORD`.
